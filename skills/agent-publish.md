---
name: agent-publish
description: 引导用户把 agent-script.md 发布到 WukongDojo.ai，生成平台发布信息（名称/描述/第一句话）和社区帖子。
---
...
如果文件不存在，提示："请先运行 `/agent-define` → `/agent-system-prompt` 完成 agent 脚本。"
...
【平台能力配置】（仅有第零部分B时执行，否则跳过）

□ 语音输入（如选）：WukongDojo 设置 → 开启语音输入
□ 文件上传（如选）：开启文件上传 → 设置允许类型（PDF/图片/…）→ 设置最大大小
□ 文件输出（如选）：确认输出模板已准备好，格式与第零部分B描述一致
□ 超强记忆（如选）：
   - 平台端已配置记忆槽字段（参照第四部分 · 记忆设计）
   - 首次用户起始语已写入台词（Step 1 会话开始协议）
   - 回访用户起始语已写入台词（Step 1 会话开始协议）
```
